...
/**
 * Created by Cloud on 2015-07-08.
 */
object MllibMatrix {
  def main(args: Array[String]) {
    if (args.length < 5) {
      println("usage: MllibMatrix <matA rows> <matA cols> <matB cols> " +
        "<sub-block row> <sub-block col>")
      System.exit(1)
    }
    ...
    val parallesim = if (args.length > 5) { args(5).toInt } else 192
    ...
    val indexedRowsB = sc.parallelize(0 until rowsB, parallesim)
      .map(row =>
